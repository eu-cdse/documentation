...
Within the Copernicus Dataspace ecosystem, a free of charge jupyterlab service is offered. It allows users to access and analyze Earth observation data effectively. 
...
The storage area remains available when the user logs out, and the Jupyter kernel is shut down due to inactivity. If the user is inactive for more than 15 days, a notification will be sent to the user to login to the Jupyterlab. If no action is taken, the files will be deleted from the CloudFerro cloud storage. 
