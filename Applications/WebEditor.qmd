...
8.  **Log in**
    ...
    As previously mentioned, it is necessary to log in to interact with the server. A new window will appear when attempting to log in, as demonstrated below. There are multiple methods available to authenticate with the server. Choose the option for which you possess valid credentials. Typically, the recommended choice is the Copernicus Data Space Ecosystem option. However, you also have the option to log in using EGI Check-in, where you can select your organisation account. For further information regarding various authentication methods or to seek assistance, you can always click on the "help" option at the top or contact us.
