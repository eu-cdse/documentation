...
5. **Guest**
    ...
    The *Guest* naming will be replaced with the username when logged in. The dropdown will provide an option to Log in.
...
7. **Process Editor**
   ...
   This is the editor for processes. It's recommended to work in "**Visual Model**" mode, where processing chains can be created simply by adding collections and processes and connecting them. The "**Code**" mode allows viewing the generated JSON process, typically needed only if running the process using another client library such as Python or R.
...
Users have the option to choose any one of them and enter the necessary parameters as instructed. This method makes the task considerably more straightforward than the previously mentioned workflow creation approach, as the structure is already established and users only need to input the essential parameters.
