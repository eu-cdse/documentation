...
The Copernicus Data Space Ecosystem offers a platform for sharing and discovering Earth Observation algorithms created using the openEO standard to enhance algorithm reusability. If you're keen on supporting algorithm reusability and want to share yours, you can publish it on the openEO Algorithm Plaza. This documentation is a starting guide for sharing your algorithm there, assuming you're familiar with EO data and [openEO](../APIs/openEO/openEO.qmd) concepts.

The [openEO Algorithm Plaza](https://marketplace-portal.dataspace.copernicus.eu/){target="_blank"} is a marketplace within Copernicus Data Space Ecosystem, where users can find and share various Earth Observation (EO) algorithms represented as openEO process graphs. It's a one-stop-shop where they can either share their algorithm or use existing ones.

This platform relies on standardized web interfaces, making it easy for users to access services. Within the platform, the hosted algorithms can be used through user interfaces or APIs. Here, users can seamlessly onboard their algorithms on the openEO Algorithm Plaza for further exposure to a large audience.
...
In addition to providing access to available services, the marketplace also supports users in showcasing their algorithms. To advertise your algorithms on the marketplace, they must be built using openEO. 

Once your algorithm is exposed as a service, users can 'invoke'  it with a given set of parameters. It's important to to consider your audience when preparing them. Especially for non-scientific users; you might want to avoid complex options that are difficult to understand.
