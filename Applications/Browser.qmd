...
![Fig 1: Browser start screen](_images/StartScreen.png)
...
#### Adding External WMS/WMTS Services

In addition to the Copernicus data collections, you can add your own third-party map services to the Browser and visualize their layers on the map. Both **WMS** and **WMTS** services are supported.

Open the **WMS/WMTS panel** from the collection selection area of the *Visualization* tab by clicking on the *WMS/WMTS Panel* button. To add a service, paste its service (GetCapabilities) URL into the input field and click the add button ( ![add](_images/icons/AddWMS.png) ) or press Enter.

Once a service is loaded, you can view its details, such as the description, access constraints and fees, by clicking on the info icon next to the service name. To select a service, click on its name; the active service is marked with a checkmark and its layers are listed below. You can remove a service at any time by clicking on the remove (trash) icon next to it.

In the layer list you can **search** layers by name, page through them, and choose how many layers to show per page. Click on a layer to display it on the map. For layers that have a time dimension, a date picker lets you choose an available date. On the active layer, use *Add to* to add it to the *Compare* or *Pins* panels. Queryable WMS layers can be clicked on the map to show feature information.

There are a few limitations to keep in mind. Only WMS 1.1.1 and WMTS 1.0.0 services are supported, and WMTS layers must provide a Web-Mercator (EPSG:3857) tile matrix set. The service must also be reachable over HTTPS and must allow cross-origin (CORS) access. Finally, time selection is limited to a single date, and the product search functionality is not available for external layers.
