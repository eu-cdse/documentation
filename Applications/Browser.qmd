...
Open the **WMS/WMTS panel** from the collection selection area of the *Visualization* tab by clicking on the *WMS/WMTS Panel* button. To add a service, paste its service (GetCapabilities) URL into the input field and click the add button ( ![add](_images/icons/AddTo.png) ) or press Enter. The service and all of its layers then appear alongside the standard collections.

Once a service is loaded, you can:

1. **View service details** (description, access constraints and fees) by clicking on the info icon next to the service name.
2. **Select** a service by clicking on its name; the active service is marked with a checkmark and its layers are listed below.
3. **Remove** a service by clicking on the remove (trash) icon.
...
- WMS 1.1.1 and WMTS 1.0.0 services are supported. WMTS layers must provide a Web-Mercator (EPSG:3857) tile matrix set.
- The service must be reachable over HTTPS and must allow cross-origin (CORS) access.
- Time selection is single-date only, and product search is not available for external layers.
