...
Users can use the services offered in the openEO Algorithm Plaza to analyse their data. These services consist out of openEO-based User Defined Processes (UDP). They can be used with the tools provided by openEO. Below is a concise overview of the various tools available:
...
Users have the flexibility to use their preferred tool to execute the services. Each service is equipped with a `Get Started` button within the plaza. This button offers available execution options for the service, such as:
...
There is a limitation when executing a service in Copernicus Data Space Ecosystem, that it only works collection from 2017 or so onwards. Moreover, it is recommended to test it for multiple consecutive years.

:::

## Online user interface

When a user excecutes a service in the webeditor using `Execute in Web Editor` option, they are provided with an online user interface. Here users can execute services directly in a [web-editor](https://openeo.dataspace.copernicus.eu/){target='_blank'} by simply inputing the required parameters and running them. 
...
Most of the services in the openEO Algorithm Plaza can be executed programmatically using the openEO client libraries. When publishing the service, the developer is requested to provide code examples for executing the service using Python, R, or JavaScript. When users of the service select the option to `See client examples`, they are directed to the provided example.

For a detailed documentation on how to use the client libraries, please refer to the official in the following links:
