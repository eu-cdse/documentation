...
---
The openEO Algorithm Plaza also provides a self-service onboarding wizard that enables developers to publish their services effortlessly. It assigns a [service maturity](ServiceMaturity.qmd) label to each onboarded service.

Developers can publish their algorithm as a service in the openEO Algorithm Plaza. These algorithms are expected to address Earth Observation solutions and implement them using openEO. When set as public, the service is then available to all users in the same way as any other service in the Algorithm Plaza. 

![](../_images/PlazaManageService.png)

Developers are directed to the described screen when "Services" is selected from the sub-menu options under the "Dashboard" section of the portal. If no services have been published yet, an option to "REGISTER YOUR FIRST SERVICE" is displayed. However, if services have already been published, a list of the services published by the developer or their organization is shown.


> For developers who are members of multiple organizations and intend on publishing an algorithm from a specific organization, it is important to ensure they are logged in to that particular organization. This can be verified by navigating to the [profile page](ManageOrg.qmd) and, if needed, [switching](ManageOrg.qmd#switch-between-organisation) the 'Linked Organization'.

![](../_images/services_list.png)

Already created services display an avatar, the service name, and a service status indication. Developers can click on the avatar or the service's name to load its detail page. Additionally, they can click on the "Register service" CTA button to load the add service page and create a new service.

Furthermore, together with the service cards created, a card with a `REGISTER NEW SERVICE` CTA button is available. This button will load the add service page to create a new service.
...
To get familiar with openEO, it is recommended to start with the [basic introduction](https://documentation.dataspace.copernicus.eu/APIs/openEO/openEO.html){target="_blank"} on using openEO. Developers can further share their algorithms as a service in the openEO Algorithm Plaza. Furthermore, to deploy an openEO algorithm as a service, the ['user defined process'](https://open-eo.github.io/openeo-python-client/udp.html){target="_blank"} functionality can be used.


## Register and publish your service

Once an algorithm is ready, developer can register and publish it. 
...
The following table outlines the basic information fields required for the service at this step::
...
In addition to the basic information mentioned above, developer can provide more details on their service by adding additional sections. The following sections are available:
...
* Results: Service's results and it is output format should be mentioned.
...
Multiple labels can be added to the service to assist users find the service within the Plaza. These labels serve as filters within the marketplace and also indicate the service's category.

##### Step 5: Select service visibility

The service can be designated as either public or private. If set to public, it will be visible to all users in the openEO Algorithm Plaza. On the other hand, if set to private, only the developer and organization members will have access to the service.
