...
  
- Service metadata should include information on detailed descriptions of the services, their parameters and a link to a publication that supports the methodology adopted. An example of the expected outcome from running service should be provided similarly to that of incubating service.

- Approximate cost estimation on a larger scale should be presented.

::: {.callout-note}

Please mention or provide a report to the support team if any constraints/limitations with the services exist that should be considered.
...
Although services can be either verified or validated, i.e., if a service satisfies all the criteria mentioned under verified but does not provide a validation report despite being relevant to them, it will be labelled as incubating.
