...
![](_images/VisualizeLatestInfo.png)
...
![](_images/Configuration_dropdown.png)
...
![Search tab](_images/SearchResult.png) Fig 11: *SEARCH* tab with L2A collection selected and map centred on Rome (Italy)
...
3.  Change the settings on the left side to:
    ...
    2.  Select 1 image per: month Alternatively, you can select only certain months in a year using the filter by months option. Click on Search to see all the results.
...
![](_images/Browser_Spectral_Explorer.png)
...
2.  Outline an agricultural field with the AOI tool to select the area you want to analyse the changes in.

![](_images/NDVI_TimeSeries.png)
