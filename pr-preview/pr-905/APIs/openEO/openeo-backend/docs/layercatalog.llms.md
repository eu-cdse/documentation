...
- Point to the layer configuration file(s), in one of the following ways:
  ...
  - Set the `layer_catalog_files` config field in your `GpsBackendConfig` [configuration](../../../../APIs/openEO/openeo-backend/docs/configuration.llms.md) object to a list of (absolute) paths to the layer catalog files.
...
Also see the [layercatalog.json](https://github.com/Open-EO/openeo-geopyspark-driver/blob/master/tests/layercatalog.json) used in the unit tests for more inspiration.
...
The easiest data source type is the “stac” type, which just requires a URL pointing to a STAC catalog or collection. For example, using the [example_stac_catalog](https://raw.githubusercontent.com/Open-EO/openeo-geopyspark-driver/refs/heads/master/docker/local_batch_job/example_stac_catalog/collection.json) included in the `openeo-geopyspark-driver` repository:
