...
- The **`ConfigParams` class** (in `openeogeotrellis/configparams.py`) was the first approach to have a **centralized** place for configuration parameters and improve the maintenance and overview challenges.

`ConfigParams` is mostly still based on reading and parsing environment variables. This is fine for basic things like the URL of some service or database, but it doesn’t scale very well:
...
Also look at the [default config file](https://github.com/Open-EO/openeo-geopyspark-driver/blob/master/openeogeotrellis/config/default.py) which is loaded when `OPENEO_BACKEND_CONFIG` is not set.

#### Setting values

The `GpsBackendConfig` class is defined through the popular [`attrs`](https://www.attrs.org/en/stable/) package to compactly define attributes with type hints and default values.
