...
> **TIP:**
>
> See [the official `openeo` installation docs](https://open-eo.github.io/openeo-python-client/installation.html) for more details, alternative installation procedures or troubleshooting tips.

## Exploring a back-end

For this tutorial we will use the openEO federation back-end of Copernicus Data Space Ecosystem, which is available at `https://openeofed.dataspace.copernicus.eu`. This backend also provides access to partner resources. Use `https://openeo.dataspace.copernicus.eu` to limit your access to the core CSDE offering. We establish a connection to this back-end as follows:
...
The [`Connection` object](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.connection.Connection) we created here is the central gateway to interact with the back-end:
...
EO data in openEO is organized in so-called collections, which are used as the input data for your openEO jobs (see [the glossary](../../../APIs/openEO/Glossary.llms.md#eo-data-collections) for more info). Collections can be listed and inspected programmatically:
...
However, it is often easier to browse collections through the [openEO collection listing page](../../../APIs/openEO/Collections.llms.md) or the collection listing sidebar of the [openEO Web Editor](../../../Applications/WebEditor.llms.md).

### openEO Processes

Processes in openEO are operations that can be applied on (EO) data (see the [the openEO glossary](../../../APIs/openEO/Glossary.llms.md) for more info). For example: calculate the mean of an array, mask out pixels outside a given polygon or calculate spatial aggregations. The output of one process can be used as the input of another process, and by doing so, multiple processes can be connected that way in a larger “process graph”, as illustrated in this conceptual diagram:

![](../_images/mermaid-figure-1.png)

While it is possible to programmatically list and inspect the available processes (e.g. `connection.list_processes()` with the openEO python client), it is recommended to just consult [the process listing page](../../../APIs/openEO/Processes.llms.md), the process listing sidebar of the [openEO Web Editor](../../../Applications/WebEditor.llms.md), or [the official openeo.org processes listing](https://processes.openeo.org/).
...
> **IMPORTANT:**
>
> Make sure to complete your [Copernicus Data Space Ecosystem registration](../../../Registration.llms.md) before attempting to do the authentication explained below.

Once properly registered, you will be able to authenticate your `connection` handle in your Python code with [`Connection.authenticate_oidc()`](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.connection.Connection.authenticate_oidc), just like this:
...
Find more in-depth information on authentication [here](../../../APIs/openEO/authentication.llms.md) or in the [openEO Python client documentation](https://open-eo.github.io/openeo-python-client/auth.html).
...
The first step is loading the desired slice of a data collection with [`Connection.load_collection`](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.connection.Connection.load_collection):
...
This results in a [`Datacube` object](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.datacube.DataCube) containing the “SENTINEL2_L2A” data restricted to the given spatial extent, the given temporal extend and the given bands .
...
> ```
>
> Still, it is recommended to always use the filters directly in [load_collection](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.connection.Connection.load_collection) to avoid loading too much data upfront.

### Applying processes

By applying an openEO process on a datacube, we create a new datacube object that represents the manipulated data. The standard way to do this with the Python client is to call the appropriate [`Datacube` object](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.datacube.DataCube) method. The most common or popular openEO processes have a dedicated `Datacube` method (e.g. `mask`, `aggregate_spatial`, `filter_bbox`, …). Other processes without a dedicated method can still be applied in a generic way. An on top of that, there are also some convenience methods that implement openEO processes is a compact, Pythonic interface.

For example, the [`min_time`](https://open-eo.github.io/openeo-python-client/api.html#openeo.rest.datacube.DataCube.min_time) method implements a `reduce_dimension` process along the temporal dimension, using the `max` process as reducer function:
...
> **NOTE:**
>
> Still unsure on how to make use of processes with the Python client? Visit the [official documentation on working with processes](https://open-eo.github.io/openeo-python-client/processes.html#working-with-processes).
...
For example: if you prefer a graphical, web-based interactive environment to manage and monitor your batch jobs, feel free to switch to an openEO web editor like [openeo.dataspace.copernicus.eu/](https://openeo.dataspace.copernicus.eu/) at any time. After logging in with the same account you use in your Python scripts, you should see your batch jobs listed under the “Data Processing” tab. More information on using openEO web editor is discussed [here](../../../Applications/WebEditor.llms.md).

> **TIP:**
>
> The official openEO Python Client documentation has more information on [batch job basics](https://open-eo.github.io/openeo-python-client/basics.html#managing-jobs-in-openeo) {target=“\_blank”} or [more detailed batch job (result) management](https://open-eo.github.io/openeo-python-client/batch_jobs.html)
...
If your use case can not be accomplished with the default processes of openEO, you can define a [user defined function](../../../APIs/openEO/Glossary.llms.md#user-defined-function-udf). Therefore, you can create a Python function that will be executed at the back-end and functions as a process in your process graph.
...
- [Example Jupyter Notebooks](https://github.com/eu-cdse/notebook-samples/tree/main/openeo)
