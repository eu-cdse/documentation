...
A rate limiting policy defines either how many [processing units](../../../APIs/SentinelHub/Overview/ProcessingUnit.llms.md) or HTTP requests can be used per given time period or in total. Both processing units and requests are rate limited and the level of rate limiting depends on your account (see [pricing plans](https://www.sentinel-hub.com/pricing/)).
...
[An example](https://sh.dataspace.copernicus.eu/ogc/wms/7702fda8-f583-4ae0-a581-1b34e7a6d350?service=WMS&request=GetMap&layers=TRUE-COLOR&&format=image%2Fjpeg&version=1.1.1&width=512&height=512&srs=EPSG%3A3857&bbox=1878516.407136492,6095394.383573096,1888300.3467569945,6105178.3231936) of a **WMS request** using the test user's instance.

The test user's Oauth client credentials below can be used to get an [access token](../../../APIs/SentinelHub/Overview/Authentication.llms.md), which can then be included in header of a **`process` API requests** (for examples of requests see [here](../../../APIs/SentinelHub/Process/Examples/S1GRD.llms.md)):
...
When your limits have been exceeded, we recommend implementing retries with a [exponential backoff](https://en.wikipedia.org/wiki/Exponential_backoff). An exponential backoff means that you wait for exponentially longer intervals between each retry of a single failing request.
