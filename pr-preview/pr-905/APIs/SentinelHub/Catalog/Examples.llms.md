...
``` python
url = "https://sh.dataspace.copernicus.eu/catalog/v1/"
...
``` python
url = "https://sh.dataspace.copernicus.eu/catalog/v1/collections"
response = oauth.get(url)
```
...
``` python
url = "https://sh.dataspace.copernicus.eu/catalog/v1/collections/sentinel-2-l1c/"
response = oauth.get(url)
```
...
``` python
url = "https://sh.dataspace.copernicus.eu/catalog/v1/collections/sentinel-1-grd/queryables"
response = oauth.get(url)
```
...
You can search for features on your own BYOC or Zarr collections. The functionality described above regarding GET and POST search is the same. The only difference is that you have to specify the collection id with the appropriate prefix on `collections` parameter (e.g: `byoc-<your-collection-id>` for byoc or `zarr-<your-collection-id>` for zarr). Remember that you will have to use the appropriate deployment endpoint depending on where your collection is hosted.
