...
Sentinel Hub Catalog API (or shortly "Catalog") is an API implementing the [STAC Specification](https://stacspec.org/), describing geospatial information about different data used with Sentinel Hub.

## API Reference

API Reference for Sentinel Hub Catalog is available as an [OpenAPI description](../../APIs/SentinelHub/ApiReference.llms.md#tag/catalog_core).
...
Authentication for the Catalog API works completely the same as authentication for other Sentinel Hub services, see [Authentication chapter](../../APIs/SentinelHub/Overview/Authentication.llms.md).
...
The search endpoint by default only accepts the parameters described in [OpenAPI](../../APIs/SentinelHub/ApiReference.llms.md#tag/catalog_item_search/operation/postCatalogItemSearch). The Filter extension enables users to specify an additional parameter to filter on, while searching through data.

The syntax for `filter` is [CQL2](https://docs.ogc.org/DRAFTS/21-065.html):
...
As with the `filter` attribute, `distinct` is also a collection limited to some specific properties. Information describing these properties can be found inside each collection's documentation (ex. [Sentinel-1 GRD](../../APIs/SentinelHub/Data/S1GRD.llms.md#distinct-extension)).
