...
- Store your raster data in the Zarr [format](https://zarr-specs.readthedocs.io/en/latest/v2/v2.0.html) on your own S3 bucket in the supported region.
- Zarr data must conform to Sentinel Hub data [constraints](#data-constraints).
...
- Data must be stored as a single [Zarr group](https://zarr-specs.readthedocs.io/en/latest/v2/v2.0.html#groups) that contains coordinate arrays and data arrays.
...
A python script to set a bucket policy can be downloaded [here](../../APIs/SentinelHub/Zarr/resources/python/cdse_set_bucket_policy.py).

## Creating Zarr collections

Each Zarr collection will correspond to a single Zarr group. When creating a collection:

- you need to provide the S3 bucket where you data is located,
- you can provide a storage identifier that determines the location of the S3 bucket,
- you need to provide the path in the bucket where the Zarr group resides, that is, the directory containing the `.zgroup` file,
- you need to provide the CRS in which your data is defined,
...
[Zarr Import API Examples](../../APIs/SentinelHub/Zarr/Examples.llms.md)
