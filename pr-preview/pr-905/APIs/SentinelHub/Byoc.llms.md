...
- Store your raster data in the cloud optimized geotiff (COG) [format](https://www.cogeo.org/) on your own S3 bucket in the supported region.
...
The Sentinel Hub BYOC Tool is software which can be used to prepare your data for use in Sentinel Hub. It can be run either in Docker or as a Java JAR. It takes care of the entire process; it is simple to use for simple cases but is also highly configurable allowing for more complex requirements. The same steps can be done manually and are detailed below, should you prefer or require more control over the process. Get the tool [here](https://github.com/sentinel-hub/byoc-tool).

## Converting to COG

### Constraints and settings

COGs can contain either a single band or multiple bands. For multi-band COGs we support both [planar configurations formats](https://docs.ogc.org/is/21-026/21-026.html#_planar_configuration_considerations) - chunky and planar format.
...
- The projection needs to be one of: WGS84 (EPSG:4326), WebMercator (EPSG:3857), any UTM zone (EPSG:32601-32660, 32701-32760), Europe LAEA (EPSG:3035), or Global EASE-Grid 2.0 (EPSG:6933).
...
- Files can be compressed with any of the following compression methods: DEFLATE, ZLIB, ZSTD, PIXTIFF_ZIP, PACKBITS, LZW. ZSTD is recommended. Note JPEG compression is not supported.
- Supported sample types and bit depths are the same as those supported for outputs, as well as reading unsigned integer 1, 2, 4 bit files. See also [sampleType](../../APIs/SentinelHub/Evalscript/Functions.llms.md#sampletype).
...
COGs can be generated in a single step with GDAL 3.1 or newer using the [COG raster driver](https://gdal.org/drivers/raster/cog.html). For older GDAL versions or if you want planar multi-band COGs, see [below](#older-gdal-versions-or-planar-multi-band-cogs). **Even though you can use any GDAL version, we highly recommend you use v3.1 or newer, as older versions have issues with average downsampling (see [https://gdal.org/programs/gdaladdo.html](https://gdal.org/programs/gdaladdo.html)).**

The input file must conform to the [constraints](#constraints-and-settings) regarding the projection, units per pixel, and pixel formats. To generate a COG from an input file:
...
You will have to configure your bucket to allow read access to Sentinel Hub. To do this, [update your bucket policy](https://creodias.docs.cloudferro.com/en/latest/s3/Bucket-sharing-using-s3-bucket-policy-on-Creodias.html#setting-a-policy-on-the-bucket) to include the following statement (don’t forget to replace `<bucket_name>` with your actual bucket name):
...
A python script to set a bucket policy can be downloaded [here](../../APIs/SentinelHub/Byoc/resources/python/cdse_set_bucket_policy.py).
...
- you need to provide the name of the S3 bucket where your data is stored; if you have data in CreoDIAS, prefix the bucket name by your CreoDIAS Project ID as follows: `PROJECT_ID:bucket_name` ,
- you can provide a storage identifier that determines the location of the S3 bucket,
- you can define bands, but only using BYOC API,
- you can provide the no data value using Dashboard or BYOC API.

The available values for storage identifier are `waw3-1`,`waw3-2` and `waw4-1`, where `waw3-1` is the default value.
...
To achieve this, you need to use [this endpoint](../../APIs/SentinelHub/ApiReference.llms.md#operation/updateByocCollectionById). You need to provide the new names at the top level, but leave the band properties ("source", "bandIndex", etc) and values the same. So the content of `additionalData.bands` would be:
...
The sample format is TIFF info that defines the band data type. It can be set to signed integers, unsigned integers, or floating points. Learn more about sample format [here](https://www.awaresystems.be/imaging/tiff/tifftags/sampleformat.html). These values are in BYOC defined as `INT`, `UINT` and `FLOAT`, respectively.
...
There are two ways of doing this. The easier version is using the [dashboard](https://shapps.dataspace.copernicus.eu/dashboard/#/byoc).
...
To ingest tiles via API requests instead of the dashboard, see [BYOC API reference](../../APIs/SentinelHub/ApiReference.llms.md#tag/byoc_collection) or [Python examples](../../APIs/SentinelHub/Byoc/Examples.llms.md).

### A note about changing files

While you may freely modify the data in your buckets, for it to continue to work reliably through Sentinel Hub you need to reingest tiles with changed data. You can do this in Dashboard, by clicking the "Refresh" button next to the tile, or using the API, by calling the [reingest endpoint](../../APIs/SentinelHub/ApiReference.llms.md#tag/byoc_tile/operation/reingestByocCollectionTileById) with the collection and tile id. This is needed as it will update metadata required for processing and failing to do so can result in odd behavior.
...
One way of getting a cover geometry is using the GDAL utility script [gdal_trace_outline](https://github.com/gina-alaska/dans-gdal-scripts/wiki/Gdal_trace_outline) which takes a raster and returns a cover geometry in the WKT format. This then needs to be converted to GeoJSON. In this example a single band file is traced:
...
The [Sentinel Hub BYOC Tool](#sentinel-hub-byoc-tool) can also help you update the cover geometry of existing tiles on Sentinel Hub. Use the `set-coverage` command. On Docker, get help and parameters by running: `docker run sentinelhub/byoc-tool set-coverage --help`

#### Workarounds

In case your input data is complex and cannot be adequately simply outlined it is nevertheless possible to obtain pixel-precise rendering. In this case, set the cover geometry to any which covers all the valid input pixels. The file bounding box as the default is such an example. What follows is doing the mosaicking in the custom script with the help of [dataMask](../../APIs/SentinelHub/UserGuides/Datamask.llms.md).

First, set the [mosaicking](../../APIs/SentinelHub/Evalscript/Functions.llms.md#mosaicking) parameter within `setup` to `TILE` (`mosaicking: Mosaicking.TILE`) and add the `dataMask` to the array of input bands.
...
[BYOC API Examples](../../APIs/SentinelHub/Byoc/Examples.llms.md)
