...
def main(c):
    ...
    cases = {
        ...
        "CLMStoCDSE_land": CLMSOffer,
        "CLMStoCDSE_priority": CLMSOffer,     
        "CMEMS" : CMEMSOffer,
        "CEMS": CEMSOffer,
    ...
    try: 
        ...
        for i in range(0, data_offer):
            ...
            try:
                ...
                zip = ''
            try:
                sp3 = c['summaries']['DataAvailability'][i]['sp3']
            except Exception:
                sp3 = ''
            try:
            ...
            t.append([product_id, content, eof, tqz, tar, zip, sp3, policy, catalogapi, S3path])
            note += footnotes
            headers = ["Product ID", "Content", "EOF", "TGZ", "tar","zip","sp3", "Rolling Policy","Catalog API","S3 Path"]
        ...
        for product_id in  product_ids:
            ...
            for i in range(0, int(counts[product_id])):
                ...
                try:
                    ...
                    dataset = ''
                try:
                    resolution = c['summaries']['DataAvailability'][i-1]['Resolution']
                except Exception:
                    resolution = ''    
                try:
                ...
                if product_id == "HIGH RESOLUTION LAYERS (HRL)":
                    ...
                    t.append([product_type,dataset,temporal,spatial,resolution,catalogue,odata,product_link])
                    headers = ["Product Type","Dataset","Temporal Extent","Spatial Extent","Spatial Resolution","S3 path","OData","Product Detail"]
                    note += footnotes
                    ...
                    t.append([product_type,dataset,resolution,spatial,temporal,catalogue,odata,product_link])
                    headers = ["Product Type","Dataset","Temporal Extent","Spatial Extent","Spatial Resolution","S3 path","OData","Product Detail"]
                    note += footnotes
