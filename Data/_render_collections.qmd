...
def detailblock(xy,time,stacurl,src):
    bandinfo = f"""<div class="accordian"><input type="checkbox" id="trigger"/><label for="trigger">Further details about the data collection</label><div>{xy}</div><div>{time}</div><div>{stacurl}</div><div>{src}</div></div>"""
    return bandinfo
