...
def get_subproduct(c):
    tabletitle = "Sub-Datasets"
    try:
        band_num = len(c['summaries']['subproduct'])
        t = []
        headers = ["Variable Name", "Description", "Units", "Input Bands"]
        empty_columns = [True] * len(headers)  # Track empty columns
        for i in range(band_num):
            product_data = c['summaries']['subproduct'][i]
...
for c in meta:
    ...
    subproduct_table = get_subproduct(c)
...
{detailblock(collectionID,spatial,temporal,table,subproduct_table,link_block,license_button,cite )}
