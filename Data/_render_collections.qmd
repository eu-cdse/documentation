...
def get_bandtable(c):
    tabletitle = "Spectral Layers"
    try:
        band_num = len(c['summaries']['eo:bands'])
        t = []
        headers = ["Layer Name", "Common Name", "GSD(m)", "Center Wavelength(μm)"]
        ...
        if len(headers) == 0:
            ...
            table = tabulate(t, headers=headers, tablefmt='html', floatfmt=".4f", stralign="left", numalign="center")
            # Set the minimum width of each column to 100 pixels
            table = table.replace("<table>", '<table class="table">')
            table = f"""<h5 style="position: sticky; top: 0; background-color:#f6f6f6">{tabletitle}</h5>{table}"""
    except:
        table = ""
