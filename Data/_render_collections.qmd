...
def detailblock(xy,time,stacurl,src):
    bandinfo = f"""<button onclick="toggleContent()" class="expand-collapse">Further details about the data collection</button><div id="myDIV" style="display:none;"><div id="myDIV">{xy}</div><div id="myDIV">{time}</div><div id="myDIV">{stacurl}</div><div id="myDIV">{src}</div></div></div><script>function toggleContent(){{var x = document.getElementById("myDIV");if (x.style.display === "none") {{x.style.display = "block";}} else {{x.style.display = "none";}}
}}</script>"""
    return bandinfo
...
for c in meta:
    ...
    try:
        ...
        for item in stac_extension:
            if item.startswith('http'):
                stac_url = f'<strong><u>Stac Extension:</u></strong>  <a href="{item}">{item}</a>'
    ...
    try:
        ...
        spatial = f'<strong><u>Spatial Extent:</u></strong> {spatial}'
        temporal = extent['temporal']['interval'][0]
        temporal = f'<strong><u>Temporal Interval:</u></strong> {temporal}'
    except:
        spatial = ""
        temporal = ""
...
{detailblock(spatial,temporal,stac_url,table)}
