...
def get_producttype(c):
    tabletitle = "Product Type"
    try:
        band_num = len(c['summaries']['producttype'])
        t = []
        headers = ["Variable Name", "Description", "Units", "Input Bands"]
        empty_columns = [True] * len(headers)  # Track empty columns
        for i in range(band_num):
            product_data = c['summaries']['producttype'][i]
            variable = product_data.get("variable", "")    
            description = product_data.get("description", "")  
            dimensions = product_data.get("dimensions", "")
            input_bands = product_data.get("input_bands", "")               
            row = [variable, description, dimensions, input_bands]
            t.append(row)
            # Check for empty columns
            for j, value in enumerate(row):
                if value:
...
for c in meta:
    ...
    if collectionID == "SENTINEL2_L2A":
        additional_info = baselineinfo("Sentinel2_baseline.qmd")
    else:
        pass
    ...
    #include sub-product table
    producttype_table = get_producttype(c)
...
{detailblock(collectionID,spatial,temporal,table,producttype_table,link_block,license_button,cite )}
