...
When accessing and working with the SENTINEL1_GRD data collection through openEO, users are required to perform SAR backscatter computation. If not specified, the process automatically uses default parameters not yet supported in the openEO ecosystem, resulting in a 'KeyError' with the message: "Backscatter coefficient 'gamma0-terrain' is not supported."
...
Hence, to avoid above discussed issue, users should utilize the `sar_backscatter` process with the coefficient set as 'sigma0-ellipsoid'.
