...
Please be informed that the **OData** and **OpenSearch** APIs were updated on 27th May 2024; the default timzone has been changed to UTC. 

As of 27th May 2024, all API requests without a specified timezone are treated by default as datetime provided in UTC format.

Previously, if a client did not specify a timezone in their date request, it defaulted to Warsaw local time. However, starting form **27th May 2024**, all API requests without a specified timezone default to datetime provided in UTC format.
