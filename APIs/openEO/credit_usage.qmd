...
A Copernicus General user receives 10000 free openEO credits automatically on the first of every month to explore and start using the openEO API.** Users can check their current credit balance in the [openEO Algorithm Plaza](https://marketplace-portal.dataspace.copernicus.eu/billing){target="_blank"} under the 'Billing' tab. If the provided credits are insufficient, you can [Submit a request](https://helpcenter.dataspace.copernicus.eu/hc/en-gb/requests/new){target="_blank"} for further support and guidance. 
