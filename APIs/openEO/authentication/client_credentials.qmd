...
-   The client credentials flow is supported on the
    [Copernicus Data Space Ecosystem openEO web editor](https://openeo.dataspace.copernicus.eu/).
    As mentioned above,
    this practically means that it can be used to track the progress and
    status of batch jobs created with a service account.
    It is also possible to approximate the batch job overview of the web editor
...
Which one to choose depends on the particular use case, and both cases are further explained below.
For simple, personal use cases and initial testing,
the self-service feature of the [Sentinel Hub Dashboard](https://shapps.dataspace.copernicus.eu/dashboard) is the easiest option.  
...
The Copernicus Data Space Ecosystem help center will help you setup an OIDC/OAuth clients, to get a service account based on these client credentials. 
You should file a request through the [help center](https://helpcenter.dataspace.copernicus.eu/hc/en-gb/requests/new).
...
is done by the user requesting the service account through help center.
