...
Moreover, these process graphs can be customized with parameters. Parameterized user-defined processes are flexible and can be adjusted to specific conditions by incorporating variables or inputs as parameters. This flexibility enable users to create adaptable processes suitable for various scenarios, enhancing their shareability.

Based on user's or creater's preference these (user-defined) process can be then also be shared in the [openEO algorithm plaza](https://marketplace-portal.dataspace.copernicus.eu/){target="_blank"}. You can find more information in [this page](https://documentation.dataspace.copernicus.eu/Applications/AlgorithmPlaza.html)
