...
As a rule of thumb, openEO is tested up to areas of 100x100km at 10m resolution. Larger extents may work, but we recommend to always first start with smaller areas. Also consider that large area jobs may result in outputs of multiple gigabytes, that become inconveniently large to download or handle on your local machine.

Also the temporal extent can have a large impact, so also here we recommend to start small. Note that it is possible to [increase job resources](./job_config.qmd).
