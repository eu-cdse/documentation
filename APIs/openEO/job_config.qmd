...
- `python-memory`: memory assigned to Python UDF's, sar_backscatter or Sentinel-3 data loading.
- `executor-cores`: number of CPUs per worker (executor). The number of parallel tasks is executor-cores/task-cpus. We recommend to set it to the same value as task-cpu, or you might see GDAL reading errors.
...
It is more convenient, but still in beta phase. Especially larger packages might be problematic.
