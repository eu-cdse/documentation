...
listing: 
  - id: Bursts
    contents:
        - notebook-samples/bursts/bursts_processing_on_demand_instruction.ipynb
    type: grid
    filter-ui: true
    fields: [title]
...
To facilitate the downloading of Sentinel-1 SLC Bursts, a dedicated tool has been developed to streamline the entire process. This tool is provided as a Jupyter Notebook, which guides users step by step through the procedure with detailed instructions. The tool uses processing on demand, ensuring that the bursts are processed only when requested, thus optimizing resource usage. As a final output, users will receive a `.zip` file containing the processed data.
...
The method described in the [Bursts API](https://documentation.dataspace.copernicus.eu/APIs/Sentinel-1%20SLC%20Burst.html#bursts-api) section is the recommended approach for acquiring bursts.
