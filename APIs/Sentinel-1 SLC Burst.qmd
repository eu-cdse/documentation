...
To facilitate the downloading of Sentinel-1 SLC Bursts, a dedicated tool has been developed to streamline the entire process. This tool is provided as a Jupyter Notebook, which guides users step by step through the procedure with detailed instructions. As a final output, users will receive a `.zip` file containing the processed data.
