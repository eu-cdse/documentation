...
# HTTPS Request
[https://catalogue.dataspace.copernicus.eu/resto/api/collections/CLMS/search.json?productType=burnt_area](https://catalogue.dataspace.copernicus.eu/resto/api/collections/CLMS/search.json?productType=burnt_area){target="_blank"}


# Python
```{python}
json = requests.get("https://catalogue.dataspace.copernicus.eu/resto/api/collections/CLMS/search.json?productType=burnt_area").json()
pd.DataFrame.from_dict(json['features']).head(3)
```
