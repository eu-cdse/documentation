...
All queries may be executed as simple HTTP-Get calls by typing the query in the web browser address line, using any HTTP client, e.g., curl or wget, or from inside the users' program. The database is accessible free and anonymously (open for anonymous access for everyone; no authorization is used). It may be accessed both from the internal network (virtual machines in Creodias) and from outside, e.g. your home computer. Note that the actual EO data are restricted to authorized users; only the Data Catalogue is open.
