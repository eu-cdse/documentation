...
formats](https://docs.ogc.org/is/21-026/21-026.html#_planar_configuration_considerations){target="_blank"} -
...
-   Files can be compressed with any of the following compression methods:
    DEFLATE, ZLIB, ZSTD, PIXTIFF_ZIP, PACKBITS, LZW. ZSTD is recommended.
    Note JPEG compression is not supported.
...
ZSTD compressed with 1024x1024 pixel internal tiling.
...
`gdal_translate -of COG -co COMPRESS=ZSTD -co BLOCKSIZE=1024 -co RESAMPLING=AVERAGE -co OVERVIEWS=IGNORE_EXISTING input.extension output.tiff`
...
`gdal_translate -co TILED=YES -co COPY_SRC_OVERVIEWS=YES --config GDAL_TIFF_OVR_BLOCKSIZE 1024 -co BLOCKXSIZE=1024 -co BLOCKYSIZE=1024 -co COMPRESS=ZSTD intermediate.tiff output.tiff`
...
-   you need to provide the name of the S3 bucket where your data is stored; if you have
...
The available values for storage identifier are `waw3-1`,`waw3-2` and `waw4-1`,
where `waw3-1` is the default value.
...
SIMPLE](/APIs/SentinelHub/Evalscript/Functions.qmd#mosaicking), only the data
...
First, set the
[mosaicking](/APIs/SentinelHub/Evalscript/Functions.qmd#mosaicking) parameter
