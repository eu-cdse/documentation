...
accounts.** Please refer to our [FAQ](/FAQ.qmd) on changing Copernicus user type (i.e. see [Which users are qualified for higher tier accounts?](https://documentation.dataspace.copernicus.eu/FAQ.html#which-users-are-qualified-for-higher-tier-accounts){target="_blank"} and [Which entities are qualified to get higher quotas in scope of “Copernicus Services” group and how can one ask for it?](https://documentation.dataspace.copernicus.eu/FAQ.html#which-entities-are-qualified-to-get-higher-quotas-in-scope-of-copernicus-services-group-and-how-can-one-ask-for-it){target="_blank"}) and accordingly fill out the form [Change your Copernicus user type](https://dataspace.copernicus.eu/copernicus-services-user){target="_blank"}.
...
::: {.callout-warning}
## Region Limitation
Currently, the BatchV2 API only supports object storage in the `WAW3-1` region. Please ensure your bucket is created in this region for compatibility.
:::
...
-   **DONE**: Feature was successfully processed. `<br/>`{=html}Caveat:
    If there was no data to process for this feature, the feature will
    still be marked with status `DONE` but with a \'**No data**\'
    message in the error column.
-   **FATAL**: Feature has failed X amount of times and will not be
    retried. The error column details the issue.
