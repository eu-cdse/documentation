...
-   `mosaicking` (optional) - ``{=html} defines input sample
    preparation, see [mosaicking](#mosaicking). Defaults to `SIMPLE`.
...
-   `SIMPLE` (default) - the simplest method, it flattens the mosaicked
    image so only a single sample is passed to evaluation. ``{=html}
...
-   `TILE` - this is essentially the unflattened mosaic. It contains all
    ...
    defined by the datasource. ``{=html}
