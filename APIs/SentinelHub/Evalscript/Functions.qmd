...
- `SIMPLE` (default) - the simplest method. Source pixels are mosaicked into a single scene; only a single sample is available for each output pixel. Maximum source count limits are in place to protect against excessively demanding requests.
- `ORBIT` - sources are mosaicked into one scene per orbit. One sample per orbit is available for each output pixel. Multiple scenes can therefore be present if there is more than one orbit for the provided time range and data filter.
- `TILE` - each source is a scene. Scenes contain all data available for the provided time range and data filter.

For `SIMPLE` mosaicking, the following source count limits are currently in place. This is the maximum number of products which will be mosaicked together. Requests which exceed limits will not fail, but may appear incomplete. Limits may change.

- Batch API - 2000
- Batch Statistical API - 2000
- Asynchronous Processing API - 1000
- Processing API, et al. - 200
