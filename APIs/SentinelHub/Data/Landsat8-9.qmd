...
| ---- | ---- | ---- | ---- | ---- | ---- |
| Optical bands <br/>B01 - B09 | Reflectance (unitless) | REFLECTANCE \[1\] | UINT16 | 0 - 0.4 | Higher values in infrared bands. Reflectance values can easily be above 1. |
| Thermal infrared bands <br/>B10 - B11 | Brightness temperature (kelvin) | BRIGHTNESS_TEMPERATURE \[2\] | UINT16 | 250 - 320 | Brightness temperature of roughly -20 to +50 C. Can reach outside this range in extreme environments. |
| BQA | Pixel quality assessment (unitless) | DN | UINT16 | bit-packed combination | The values can be obtained using the utility function the function named *decodeL8C2Qa* ([see](/APIs/SentinelHub/Evalscript/Utilities.qmd#decodel8c2qa)). |
...
\[1\]: Top of the Atmosphere Reflectance, which is obtained using the scene center sun angle. <br/>
\[2\]: Top of the Atmosphere Brightness Temperature.

### Scenes Object

[`scenes` object](/APIs/SentinelHub/Evalscript/Functions.qmd#scenes) stores metadata. An example of metadata available in `scenes` object for Landsat 8-9 L1 when mosaicking is `ORBIT`:
...
Properties of a `scenes` object can differ depending on the selected mosaicking and in which evalscript function the object is accessed. [Working with metadata in evalscript](/APIs/SentinelHub/UserGuides/Metadata.qmd) user guide explains all details and provide examples.
...
[Landsat 8-9 L1 examples](/APIs/SentinelHub/Process/Examples/Landsat8-9.qmd)
