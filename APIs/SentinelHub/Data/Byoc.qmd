...
## About BYOC Data {#about-byoc-data}

Bring Your Own COG (BYOC) is data ingested by Sentinel Hub users using [BYOC API](/APIs/SentinelHub/Byoc.qmd).
...
Use `byoc-<collectionId>` for BYOC collections as the value of the `input.data.type` parameter in your API requests.
...
(`TYPENAMES` parameter) to `byoc-<MyCollectionID>` e.g.
