...
the [STAC Specification](https://stacspec.org/){target="_blank"},
describing geospatial information about different data used with
Sentinel Hub.

## API Reference

API Reference for Sentinel Hub Catalog is available as an [OpenAPI
description](/APIs/SentinelHub/ApiReference.qmd#tag/catalog_core).
...
[OpenAPI](/APIs/SentinelHub/ApiReference.qmd#tag/catalog_item_search/operation/postCatalogItemSearch).
...
The available operators are `eq`, `neq`, `lt`, `lte`, `gt`, `gte` and
`between`. Only `and` is currently supported as a logical operator. Be
careful - different collections have different properties for the query
filter available. The information describing this is available inside
the documentation for each specific collection (ex. [Sentinel-1
GRD](/APIs/SentinelHub/Data/S1GRD.qmd#filter-extension)).
...
GRD](/APIs/SentinelHub/Data/S1GRD.qmd#distinct-extension)).
