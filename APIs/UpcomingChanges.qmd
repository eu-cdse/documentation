...
Please be informed that the **STAC API** items interface will be updated on the **05th of October 2023**. The update will be announced in a separate release note within [release notes document](APIs/ReleaseNotes.qmd). 
...
interface will be updated on the **02nd of October 2023**. The update will be announced in a separate release note within [release notes document](APIs/ReleaseNotes.qmd). 

To be compliant with the OData PRIP interfaces the OData API response for a single product query will be updated. The response will now return the product itself instead of a list containing the product (assigned to the field `value`). The example reflecting the change is presented below.
