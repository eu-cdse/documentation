...
Please be informed that the **STAC API** items interface will be updated on the **25th of September 2023**. The update will be announced in a separate release note within [release notes document](APIs/ReleaseNotes.qmd). 
...
[`https://catalogue.dataspace.copernicus.eu/stac/search?datetime=2023-09-01`](<https://catalogue.dataspace.copernicus.eu/stac/search?datetime=2023-09-01>){target="_blank"}
...
## OData API single product response update

Please be informed that the **OData API** 
interface will be updated on the **27th of September 2023**. The update will be announced in a separate release note within [release notes document](APIs/ReleaseNotes.qmd). 

To be compliant with the OData PRIP interfaces the API response for a single product query will be updated. The response will now return the product itself instead of a list containing the product (assigned to the field `value`). The example reflecting the change is presented below.
...
The exact date of the update will be announced [here](APIs/UpcomingChanges.qmd) in a separate release note no later than 5 days before the scheduled change.
