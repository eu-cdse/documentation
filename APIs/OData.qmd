...
OData is an SO/IEC approved, OASIS standard , which is based on http RESTful Application Programming Interfaces. It enables resources, which are identified by URLs and defined in a data model, to be created and edited using simple HTTP messages.  OData makes it possible to build REST-based data services that let Web clients publish and edit resources that are recognized by Uniform Resource Locators (URLs) and described in a data model using straightforward HTTP messages.
...
Alternatively `contains`, `endswith` and `startswith` can be used to search for products ending or starting with provided string.
...
The `orderby` option can be used to order the products in an ascending (`asc`) or descending (`desc`) direction. If `asc` or `desc` is not specified, then the resources will be ordered in ascending order.
...
```
https://zipper.dataspace.copernicus.eu/odata/v1/Products(<PRODUCT_UUID>)/Nodes
https://zipper.dataspace.copernicus.eu/odata/v1/Products(<PRODUCT_UUID>)/Nodes(<NODE_NAME>)/Nodes
https://zipper.dataspace.copernicus.eu/odata/v1/Products(<PRODUCT_UUID>)/Nodes(<NODE_NAME>)/Nodes(<NODE_NAME>)/Nodes
```
...
For available collections, please refer to [link](https://documentation.dataspace.copernicus.eu/APIs/OData.html#query-collection-of-products). Also, please note that it is possible that none of the products have been deleted from the available collections.
